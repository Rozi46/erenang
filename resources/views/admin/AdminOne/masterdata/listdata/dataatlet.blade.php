                ...
                <div class="container-fluid text-left">
                    <div class="row">
                        <div class="col-md-12 bg_page_main hd">
                            <div class="col-md-12 hd_page_main">Data Atlet</div>
							<div class="col-md-12 bg_act_page_main">
								<div class="row">
									<div class="col-xl-12 col_act_page_main text-left">
										...
										@if($level_user['newatlet'] == 'Yes')<a load="true" href="/admin/newatlet"><button type="button" class="btn btn-primary">Tambah Data</button></a>@endif
										
										@if($level_user['exportlistatlet'] == 'Yes')<button type="button" class="btn btn-info back" onclick="exportdata('listatlet')"><i class="fa fa-download"></i> Export Data</button>@endif
									</div>
								</div>
							</div>
                        </div>
										...
										<table class="table_view table-striped table-hover">
											<thead>
												<tr>
													...
													<th style="min-width:150px; text-align: center;">NIS</th>
													<th style="min-width:150px; text-align: center;">Nama</th>
													<th style="min-width:100px; text-align: center;">Gender</th>
													<th style="min-width:100px; text-align: center;">Tempat Lahir</th>
													<th style="min-width:100px; text-align: center;">Tanggal Lahir</th>
													<th style="min-width:100px; text-align: center;">Nama Club</th>
											...
											<tbody>
												<?php $no = 0;?> @forelse($results['data'] as $view_data) 
																...
																if($('[btn="del_data_{{$view_data['code_data']}}"]').click){
																	...
																	$('button[btn-action="action-confirmasi"]').click(function(){
																		if($('button[btn-action="action-confirmasi"]').click){
																			...
																			loadingpage(20000);
																			window.location.href = "/admin/deleteatlet?d={{$view_data['code_data']}}";
																		}
																	});
																}
															});
													...
													<tr>
														...
														<td style="text-align:center;">{{$view_data['nis'] ?? 'Belum ditentukan'}}</td>
														<td style="text-align:left;">{{$view_data['nama'] ?? 'Belum ditentukan'}}</td>
														<td style="text-align:center;">{{$view_data['gender'] ?? 'Belum ditentukan'}}</td>
														<td style="text-align:center;">{{$view_data['tempat_lahir'] ?? 'Belum ditentukan'}}</td>
                                                        <td style="text-align:center;">{{ !empty($view_data['tanggal_lahir']) ? Carbon::parse($view_data['tanggal_lahir'])->translatedFormat('d F Y') : 'Belum ditentukan' }}</td>
														<td style="text-align:center;">{{$view_data['club']['nama_club'] ?? 'Belum ditentukan'}}</td>
														...
														<td class="colright" style="text-align:center;">
															<div class="dropdown dropleft">
																...
																<div class="dropdown-menu">
																	...
																	<a load="true" class="dropdown-item" href="/admin/editatlet?d={{$view_data['code_data']}}">Lihat/Ubah Data</a>
																	<a class="dropdown-item @if($view_data['registrasi_count'] > 0) disabled @endif @if($level_user['deleteatlet'] == 'No') disabled @endif" <?php if($view_data['registrasi_count'] == 0){ if($level_user['deleteatlet'] == 'Yes'){ ?> btn="del_data_{{$view_data['code_data']}}"<?php } }?>>Hapus Data</a>
																</div>
															</div>
														</td>
													</tr>
