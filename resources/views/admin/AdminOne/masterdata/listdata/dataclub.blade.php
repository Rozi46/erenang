                ...
                <div class="container-fluid text-left">
                    <div class="row">
                        <div class="col-md-12 bg_page_main hd" line="hd_action">
                            <div class="col-md-12 hd_page_main">Data Club</div>
							<div class="col-md-12 bg_act_page_main">
								<div class="row">
									<div class="col-xl-12 col_act_page_main text-left">
										...
										@if($level_user['newclub'] == 'Yes')<a load="true" href="/admin/newclub"><button type="button" class="btn btn-primary">Tambah Data</button></a>@endif
										
										@if($level_user['exportlistclub'] == 'Yes')<button type="button" class="btn btn-info back" onclick="exportdata('listclub')"><i class="fa fa-download"></i> Export Data</button>@endif
									</div>
								</div>
							</div>
                        </div>
										...
										<table class="table_view table-striped table-hover">
											<thead>
												<tr>
													...
													<th style="min-width:250px; text-align: center;">Nama Club</th>
													<th style="min-width:250px; text-align: center;">Kota Asal</th>
													<th style="min-width:250px; text-align: center;">Kontak</th>
											...
											<tbody>
												<?php $no = 0;?> @forelse($results['data'] as $view_data) 
														...
														$(document).ready(function(){
															$('[btn="del_data_{{$view_data['code_data']}}"]').click(function(){
																if($('[btn="del_data_{{$view_data['code_data']}}"]').click){
																	...
																	$('div[data-model="confirmasi"] .modal-body').html('<div class="alert alert-danger">Anda yakin untuk menghapus data {{$view_data['nama_club']}}.</div>');
																	...
																	$('button[btn-action="action-confirmasi"]').click(function(){
																		if($('button[btn-action="action-confirmasi"]').click){
																			...
																			loadingpage(20000);
																			window.location.href = "/admin/deleteclub?d={{$view_data['code_data']}}";
																		}
																	});
																}
															});
													...
													<tr>
														...
														<td style="text-align:left;">{{$view_data['nama_club'] ?? 'Belum ditentukan'}}</td>
														<td style="text-align:left;">{{$view_data['kota_asal'] ?? 'Belum ditentukan'}}</td>
														<td style="text-align:center;">{{$view_data['kontak'] ?? 'Belum ditentukan'}}</td>
														...
														<td class="colright" style="text-align:center;">
															<div class="dropdown dropleft">
																...
																<div class="dropdown-menu">
																	...
																	<a load="true" class="dropdown-item" href="/admin/editclub?d={{$view_data['code_data']}}">Lihat/Ubah Data</a>
																	<a class="dropdown-item @if($view_data['atlet_count'] > 0) disabled @endif @if($level_user['deleteclub'] == 'No') disabled @endif" <?php if($view_data['atlet_count'] == 0){ if($level_user['deleteclub'] == 'Yes'){ ?> btn="del_data_{{$view_data['code_data']}}"<?php } }?>>Hapus Data</a>
																</div>
															</div>
														</td>
													</tr>
