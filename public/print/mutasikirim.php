<?php
    ...
    if(isset($_REQUEST['token'])){
        ...
        if($get_user['status_message'] == 'failed'){
            ...
            $get_data = curlget($url_api."v1/viewmutasikirim?token=".$key_token."&u=".$admin_login."&code_data=".$print_code);
            ...
            if($get_data['status_message'] == 'failed'){
                ...
                class PDF extends MultiCellTable
                {
                    ...
                    function Header()
                    {
                        ...
                        $get_data = curlget($url_api."v1/viewmutasikirim?token=".$key_token."&u=".$admin_login."&code_data=".$print_code."&tipe_data=group");
                        $get_data = json_decode($get_data, TRUE);
                        $getdata = $get_data['results'];
                        ...
                        $this->setFont('Arial', '', 9);
                        $this->Cell(5);
                        $this->MultiCell($setw40, $seth, 'No. Mutasi', 0);
                        $getx += $setw40;
                        $this->SetXY($getx, $gety);
                        $this->MultiCell($setw5, $seth, ':', 0);
                        $getx += $setw5;
                        $this->SetXY($getx, $gety);
                        $this->MultiCell($setw40, $seth, ($getdata['detail']['nomor'] ?? 'Belum ditentukan'), 0);
                        ...
                        $this->SetXY($getx, $gety);
                        $this->Cell(16.5);
                        $this->MultiCell($setw45, $seth, ($getdata['detail_gudang_asal']['nama'] ?? 'Belum ditentukan'), 0);
                        $getx += $setw45;
                        $this->SetXY($getx, $gety);
                        ...
                        $this->setFont('Arial', '', 9);
                        $this->Cell(5);
                        $this->MultiCell($setw40, $seth, 'Tanggal', 0);
                        $getx += $setw40;
                        $this->SetXY($getx, $gety);
                        $this->MultiCell($setw5, $seth, ':', 0);
                        $getx += $setw5;
                        ...
                        $this->Cell(17.5);
                        $this->MultiCell($setw30, $seth, 'Gudang Tujuan', 0);
                        $getx += $setw30;
                        $this->SetXY($getx, $gety);
                        $this->Cell(17.5);
                        $this->MultiCell($setw5, $seth, ':', 0);
                        ...
                        $this->SetXY($getx, $gety);
                        $this->Cell(16.5);
                        $this->MultiCell($setw30 + 10, $seth, ($getdata['detail_gudang_tujuan']['nama'] ?? 'Belum ditentukan'), 0);
                        $getx += $setw45;
                        $this->SetXY($getx, $gety);
                    ...
                    function Footer()
                    {
                        if($this->isFinished){
                            ...
                            $this->Line(11.5,$gety-0.5,197,$gety-0.5);
                            ...
                            $this->SetFont('Arial', '', 9);
                            $this->Cell(12);
                            $this->MultiCell(148, 4, ($getdata['detail']['ket'] ?? 'Belum ditentukan'), 0);
                            ...
                            $this->Cell(12);
                            $this->SetFont('Arial', '', 9);
                            $this->MultiCell(178.5, 3.5, ($getdata['detail_gudang_tujuan']['alamat'] ?? 'Belum ditentukan'), 0, 'L');
                            ...
                            $this->Ln(18);
                            $this->Cell(95, 5, '( ' . ($getdata['user_transaksi']['full_name'] ?? 'Belum ditentukan') . ' )', 0, 0, 'C');
                ...
                $title = 'Print Mutasi Kirim | PERDANA MOTOR';
                $pdf->SetTitle($title);
                ...
                // for ($i = 0; $i < 30; $i++) {
                    foreach ($getdata['list_produk'] as $view_data) {
                        $id = $view_data['id'];
                        $id = str_replace('-','',$id);
                        $nm_prod = ($get_data['results']['detail_produk'][$view_data['kode_barang']]['nama'] ?? 'Belum ditentukan');
                        $qty_kirim = ($view_data['qty'] ?? 0);
                        ...
                        $pdf->Row(array(
                            array($no,'C'),
                            array($nm_prod,'L'),
                            array(number_format($qty_kirim,0,"",".").' '.($get_data['results']['satuan_produk'][$view_data['kode_barang']]['nama'] ?? 'Belum ditentukan'),'C'),
                        ));
                        ...
                        $pdf->Line(169,$gety,169,$getx_ln);    // Garis di antara kolom kedua dan ketiga
                        ...
                        $space_left = $page_height - ($pdf->GetY() + $bottom_margin)+ 35;
                ...
                // Table rows (with 70 iterations)
                // for ($i = 0; $i < 70; $i++) {
                //     // Simulasi data produk
                //     $id = "PROD-" . str_pad($i + 1, 4, '0', STR_PAD_LEFT); // Buat id produk
                //     $nm_prod = "Product " . ($i + 1); // Nama produk fiktif
                //     $qty_kirim = rand(1, 100); // Kuantitas fiktif (acak antara 1 hingga 100)
                //     $satuan = "Pcs"; // Satuan produk

                //     // Get current Y position
                //     // $gety = $pdf->GetY();

                //     // Set tinggi cell dan posisi garis mendatar (horizontal)
                //     // $height_of_cell = 5;
                //     // $getx_ln = $gety + $height_of_cell;
                ...
                //         array($i + 1, 'C'),
                //         array($nm_prod, 'L'),
                //         array(number_format($qty_kirim, 0, "", ".") . ' ' . $satuan, 'C'),
                //     ));
                ...
                //     // Check for page break after each row
                //     $pdf->CheckPageBreak(6.5);
                // }
