<?php
    ...
    if(isset($_REQUEST['token'])){
        ...
        if($get_user['status_message'] == 'failed'){
            ...
            $get_data = curlget($url_api."v1/viewpurchasepayment?token=".$key_token."&u=".$admin_login."&code_transaksi=".$print_code."&tipe_data=group");
            ...
            if($get_data['status_message'] == 'failed'){
                ...
                class PDF extends MultiCellTable
                {
                    function Header()
                    {
                        ...
                        $this->Ln(7);
                        $this->Cell(115);
                        $this->MultiCell($setw30,$seth,'Purchases Order',0);
                        $getx+=$setw30;
                        $this->SetXY($getx, $gety);
                        $this->Ln(7);
                        $this->Cell(145);
                        ...
                        $this->Ln(7);
                        $this->Cell(149);
                        $this->MultiCell($sethfull,$seth,($getdata['detail']['nomor_hutang'] ?? 'Belum ditentukan'),0);
                        $getx+=$setw45;
                        $this->SetXY($getx, $gety);
                        ...
                        $this->Cell(12);
                        $this->MultiCell($sethfull,$seth,($getdata['detail_supplier']['nama'] ?? 'Belum ditentukan'),0);
                        $this->setFont('Arial','',9);
                        $this->Cell(12);
                        $this->MultiCell($sethfull,$seth,($getdata['detail_supplier']['alamat'] ?? 'Belum ditentukan'),0,'L');
                        $this->Cell(12);
                        ...
                        $this->MultiCell($setw70,$seth,($getdata['detail_supplier']['no_telp'] ?? 'Belum ditentukan'),0);
                        $getx+=$setw70 - 7.5;
                        $this->SetXY($getx, $gety);
                ...
                $pdf->Line(11.5, $startY, 197, $startY);

                // Header Amount
                $pdf->Ln(2);
                $pdf->SetFont('Arial', 'B', 10);
                $pdf->Cell(10); // Indentasi dari margin kiri
                $pdf->Cell(45, 5, 'Amount', 0, 0, 'L'); // Label "Amount"
                $pdf->Cell(4, 5, ':', 0, 0, 'L');       // Separator ":"
                $pdf->MultiCell(125, 5, number_format($getdata['detail']['jumlah'] ?? 0, 2, ",", "."), 0, 'L'); // Nilai Amount

                // Garis bawah Amount
                $amountY = $pdf->GetY();
                $pdf->SetLineWidth(0.3);
                $pdf->Line(65, $amountY, 190, $amountY); // Garis bawah untuk nilai Amount

                // Header Terbilang
                $pdf->Ln(2); // Spasi antar baris
                $pdf->SetFont('Arial', 'IB', 10);
                $pdf->Cell(10); // Indentasi dari margin kiri
                $pdf->Cell(45, 5, '', 0, 0, 'L');       // Kosong untuk label (tidak perlu)
                $pdf->Cell(4, 5, ':', 0, 0, 'L');       // Separator ":"
                $pdf->MultiCell(125, 5, terbilang($getdata['detail']['jumlah'] ?? 0), 0, 'L'); // Nilai terbilang
                
                // Garis bawah Terbilang
                $terbilangY = $pdf->GetY();
                $pdf->SetLineWidth(0.3);
                $pdf->Line(65, $terbilangY, 190, $terbilangY);
                
                // Garis Bawah Keseluruhan
                $endY = $pdf->GetY();
                $pdf->SetLineWidth(0.3);
                $pdf->Line(11.5, $endY + 5, 197, $endY + 5);
                $pdf->Line(11.5, $startY, 11.5, $endY + 5);
                $pdf->Line(197, $startY, 197, $endY + 5);
                
                $pdf->Ln(10);
                
                // Tanda Tangan Receiver
                $pdf->SetFont('Arial', '', 10);
                $pdf->Cell(30);
                $pdf->Cell(20, 5, 'Receiver By', 0, 0, 'C');
                $pdf->Ln(25);
                $pdf->Cell(30);
                $pdf->Cell(20, 5, '( _______________ )', 0, 0, 'C');
                
                // Tanda Tangan Approved
                $pdf->Ln(-25);
                $pdf->SetFont('Arial', '', 10);
                $pdf->Cell(150);
