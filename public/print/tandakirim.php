<?php
    ...
    if(isset($_REQUEST['token'])){
        ...
        if($get_user['status_message'] == 'failed'){
            ...
            $get_data = curlget($url_api."v1/viewpengiriman?token=".$key_token."&u=".$admin_login."&nomor_pengiriman=".$print_code);
            ...
            if($get_data['status_message'] == 'failed'){
                ...
                class PDF extends MultiCellTable
                {
                    ...
                    function Header()
                    {
                        ...
                        $get_data = curlget($url_api."v1/viewpengiriman?token=".$key_token."&u=".$admin_login."&nomor_pengiriman=".$print_code."&tipe_data=group");
                        $get_data = json_decode($get_data, TRUE);
                        $getdata = $get_data['results'];
                        ...
                        $this->SetFont('Arial','B',14);
                        $this->Ln();
                        $this->Cell(0,5,'PENGIRIMAN BARANG',0,0,'C');
                        $this->Ln(10);
                        ...
                        $this->setFont('Arial','',9);
                        $this->Cell(5);
                        $this->MultiCell($setw40,$seth,'Delivery Order',0);
                        $getx+=$setw40;
                        $this->SetXY($getx, $gety);
                        $this->MultiCell($setw5,$seth,':',0);
                        $getx+=$setw5;
                        $this->SetXY($getx, $gety);
                        $this->MultiCell($setw40,$seth,($getdata['detail']['nomor_pengiriman'] ?? 'Belum ditentukan'),0);
                        ...
                        $this->SetXY($getx, $gety);
                        $this->Cell(16.5);
                        $this->MultiCell($setw45,$seth, ($getdata['detail_penjualan']['nomor'] ?? 'Belum ditentukan'),0);
                        $getx+=$setw45;
                        $this->SetXY($getx, $gety);
                        ...
                        $this->SetXY($getx, $gety);
                        $this->Cell(16.5);
                        $this->MultiCell($setw30 + 10,$seth,isset($getdata['detail_penjualan']['tanggal']) ? date('j F Y',strtotime($getdata['detail_penjualan']['tanggal'])) : 'Belum ditentukan',0);
                        $getx+=$setw45;
                        $this->SetXY($getx, $gety);
                        ...
                        $this->setFont('Arial','',9);
                        $this->Cell(5);
                        $this->MultiCell($setw40,$seth,'Customer',0);
                        $getx+=$setw40;
                        $this->SetXY($getx, $gety);
                        $this->MultiCell($setw5,$seth,':',0);
                        $getx+=$setw5;
                        $this->SetXY($getx, $gety);
                        $this->MultiCell($setw45 + 8,$seth,($getdata['detail_customer']['nama'] ?? 'Belum ditentukan'),0);
                        $setmulti_h = $this->GetY();
                        $setmulti_h= $setmulti_h-$gety;
                        $getx+=$setw45;
                        $this->SetXY($getx, $gety);
                    ...
                    function Footer()
                    {
                        if($this->isFinished){
                            ...
                            $this->setX(11.5);
                            $this->setFont('Arial','B',9);
                            $this->MultiCell(185.5,$hmt_add,'Delivery Address :',0);
                            $this->setX(14.5);
                            $this->setFont('Arial','',9);
                ...
                foreach ($getdata['list_produk_group'] as $view_data) {
                    ...
                    $qty_kirim = ($view_data['jumlah_kirim'] ?? 0);
                    ...
                    $pdf->Row(array(
                        array($no,'C'),
                        array($nm_prod,'L'),
                        array(number_format($qty_kirim,0,"",".").' '.($get_data['results']['satuan_produk'][$id]['nama'] ?? 'Belum ditentukan'),'C'),
                    ));
                ...
                $pdf->Output('I','Print-'.$getdata['detail']['nomor_pengiriman'].'.pdf');
            }
        }
    }else{
        echo "<meta http-equiv='refresh' content='0;/'>";
