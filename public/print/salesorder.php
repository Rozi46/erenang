<?php
    ...
    if(isset($_REQUEST['token'])){
        ...
        if($get_user['status_message'] == 'failed'){
            ...
            $get_data = curlget($url_api."v1/viewpenjualan?token=".$key_token."&u=".$admin_login."&code_data=".$print_code);
            ...
            if($get_data['status_message'] == 'failed'){
                ...
                {  
                    ...
                    function __construct()
                    {
                        ...
                        $get_data = curlget($url_api."v1/viewpenjualan?token={$key_token}&u={$admin_login}&code_data={$print_code}&tipe_data=group");
                        $get_data = json_decode($get_data, TRUE);
                    ...
                    function Header()
                    {
                        ...
                        $jenis = strtolower($this->getdata['detail_perusahaan']['jenis'] ?? 'Belum ditentukan');
                        $jenis = ucwords($jenis);

                        $alamat = strtolower($this->getdata['detail_perusahaan']['alamat'] ?? 'Belum ditentukan');
                        ...
                        $this->Ln(3);
                        $this->Cell(45);
                        $this->Cell(0,5,strtoupper($this->getdata['detail_perusahaan']['kantor'] ?? 'Belum ditentukan'),0,0,'L');
                        $this->Ln(6);
                        ...
                        $this->Ln();
                        $this->Cell(45.5);
                        $this->Cell(0,5,'Email : ' .$this->getdata['detail_perusahaan']['email'] ?? 'Belum ditentukan',0,0,'L');
                        ...
                        $this->Cell(1);
                        $this->MultiCell($setw30,$seth,'Sales No',0);
                        $getx+=$setw30;
                        $this->SetXY($getx, $gety);
                        $this->MultiCell($setw5,$seth,':',0);
                        $getx+=$setw5;
                        ...
                        foreach ($this->getdata['list_produk'] as $view_data) {
                            ...
                            for ($i = 1; $i <= 1; $i++) {                                
                                ...
                                $this->Row([
                                    ...
                                    array(number_format($view_data['jumlah_jual'] ?? 0,0,"",".").' '.($this->getdata['satuan_produk'][$no_data]['nama'] ?? 'Belum ditentukan'),'C'),
