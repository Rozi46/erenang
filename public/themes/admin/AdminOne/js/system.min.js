...
function root$(){
    return window.parent && window.parent.$ ? window.parent.$ : $;
}

/* ================= ALERT ================= */
window.SystemAlert = function(type, message){
    const $ = root$();
    const box = type==='success'
        ? $('[line="alert_success"]')
        : $('[line="alert_danger"]');

    if(!box.length) return;

    box.text(message).stop(true,true).fadeIn(200);

    setTimeout(()=>box.fadeOut(300),4000);
};

/* ================= TOAST ================= */
window.SystemToast = function(type, message){
    const $ = root$();
    const toast = $('<div class="toast-system"></div>')
        .addClass('toast-'+type)
        .text(message);

    $('#globalToast').append(toast);
...
/* ================= CONFIRM ================= */
window.SystemConfirm = function(message, callback){
    const $ = root$();
